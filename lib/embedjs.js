...
SteemEmbed.youtube = function (url, id, options) {
  var timerMatches = url.match(/[?&]t=([0-9hms]+)/);
  var autoplayValue = options.autoplay ? 1 : 0;
  var srcUrl = '//www.youtube.com/embed/' + id + '?autoplay=' + autoplayValue;
  if (timerMatches && timerMatches[1]) {
    srcUrl += '&start=' + timerMatches[1];
  }
